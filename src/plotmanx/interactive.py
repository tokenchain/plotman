...
def curses_main_v2(stdscr):
    ...
    mini = MintJ(cfg.plotting)
    mini.GenJobs(cfg.directories)
    jobs = mini.LsJobs
    last_refresh = None
    ...
    while True:
        ...
        if not do_full_refresh:
            mini.CacheGenJobs(cfg.directories)
            jobs = mini.LsJobs
            ...
            if plotting_active and mini.isClockReady:
                for i in range(cfg.scheduling.parallel):
                    ...
                    if started:
                        ...
                        jobs = Job.genTasks(cfg.directories.log, cached_jobs=jobs)
                    else:
