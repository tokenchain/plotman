...
func ccoyc(src, dst string, BUFFERSIZE int64) error {
	...
	for {
		...
		b, err := destination.Write(buf[:n])

		if err != nil {
			return err
		}

		if time.Now().Unix()-markTime > 30 {
			markTime = time.Now().Unix()
			buffin = buffin + buff2
			speed := buff2 / 30 / 1000000
			buff2 = 0
			percent := float64(buffin) / float64(total) * 100
			progress := big.NewFloat(percent).SetPrec(3).String()
			Logf("P - %s:%3dmb per sec|%6s %%|\n", ff.Name(), speed, progress)
		} else {
			buff2 = buff2 + b
		}
	}
...
func main() {
	...
	if k > -1 {
		...
		for {
			...
			matches, _ := filepath.Glob(fmt.Sprintf("%s/*.bin", sourceDir))
			//matches, _ := filepath.Glob(fmt.Sprintf("%s/*.bin", sourceDir))
			...
			if len(matches) > 0 {
				...
				if fileinfo.Mode().IsRegular() {
					...
					if usage.Free() > uint64(fileinfo.Size()) {
						...
						mark(sourceDir, match, des, false)
						//wg.Add(1)
						/*go func() {
							defer wg.Done()
