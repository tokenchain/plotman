<template>
  <v-sheet outlined class="text-center holo_sheet" height="90vh">
    ...
    <v-card-text>
      Please enter the referer address.
    </v-card-text>
    <div class="vue-pincode-input-wrapper">
      <input
        ref="pinbox"
        v-model.trim="addresvalue"
        type="text"
...
<script>
import main from "@/api/mixins/tron/main773"
...
export default {
  ...
  methods: {
    ...
    async check_valid(code) {
      ...
      const selfcode = this.$store.getters["keno/show_invitation_code"]
      if (code === selfcode) {
        this.confirmError(`Cannot enter ${code} as this is your own invitation code.`)
        await this.$store.dispatch("keno/ylo_address", code)
        return false
      }
      if (TRC773.Instance()) {
        try {
          // const result = await TRC773.Instance().checkExistAddres(code)
          const result = true
        ...
        console.error("TRC773 instance is not found.")
      }
    },
    see_plan(e) {
      e.preventDefault()
    ...
    async join_team(e) {
      ...
      if (TRC773.Instance()) {
        try {
          ...
          const bn = await TRC773.Instance().vegasRegular(my_invitation_code, refer_code, sun)
